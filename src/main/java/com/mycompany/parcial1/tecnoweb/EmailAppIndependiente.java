...
public class EmailAppIndependiente implements ICasoUsoListener {
    ...
    public void processEmailCommand(String senderEmail, String subject, String content) {
        try {
            ...
            if (!TestConnection.testConnection()) {
                ...
                sendErrorEmail(senderEmail, "Error de conexión a la base de datos. Verifica que PostgreSQL esté ejecutándose.");
                return;
            }
            
            // Verificar si el comando es para registro
            if (isRegistrationCommand(subject)) {
                processRegistrationCommand(senderEmail, subject);
                return;
            }
     ...
     */
    private void processRegistrationCommand(String senderEmail, String subject) {
        try {
            String[] parts = subject.trim().split("\\s+");
            
            if (parts.length < 5) {
                sendRegistrationHelp(senderEmail, "❌ Faltan parámetros para el registro");
                return;
            }
            ...
            sendRegistrationHelp(senderEmail, "❌ Error en el registro: " + e.getMessage());
        }
    }
     ...
     */
    private void sendRegistrationHelp(String email, String error) {
    ...
    /**
     * Envía email de bienvenida para usuarios no registrados
     */
    ...
    private String normalizeCommand(String entity) {
        ...
        switch (entity) {
            ...
            case "tipos_pago": case "tipo_pagos": return "tipo_pago";
    ...
    private void processDirectCommand(String senderEmail, String comando) {
        try {
            ...
            if (parts.length == 0) {
                sendErrorEmail(senderEmail, "Comando vacío");
                return;
            }
            ...
            System.out.println("   🎯 Entidad: " + entity + " (original: " + parts[0] + "), Acción: " + action + ", Parámetro: " + param);
            
            switch (entity) {
                case "usuario":
                    processUsuarioCommand(senderEmail, action, param, comando);
                    break;
                case "producto":
                    processProductoCommand(senderEmail, action, param, comando);
                    break;
                case "categoria":
                    processCategoriaCommand(senderEmail, action, param, comando);
                    break;
                case "cliente":
                    processClienteCommand(senderEmail, action, param, comando);
                    break;
                case "tipo_pago":
                    processTipoPagoCommand(senderEmail, action, param, comando);
                    break;
                case "help":
                    processHelpCommand(senderEmail, comando);
                    break;
                default:
                    sendErrorEmail(senderEmail, "Comando no reconocido: " + parts[0] + " (intenta: usuario, producto, categoria, cliente, tipo_pago, help)");
            }
                ...
                if (param != null) {
                    try {
                        ...
                        sendTableResponse(senderEmail, "Usuario encontrado", 
                            DUsuario.HEADERS, (ArrayList<String[]>) userData, comando);
                    } catch (NumberFormatException e) {
                        sendSimpleResponse(senderEmail, "Error", "ID inválido: " + param);
                    }
                } else {
                    List<String[]> usuarios = nUsuario.list();
                    sendTableResponse(senderEmail, "Lista de Usuarios", 
                        DUsuario.HEADERS, (ArrayList<String[]>) usuarios, comando);
                }
            } else {
                sendSimpleResponse(senderEmail, "Comando no disponible", 
                    "Solo 'usuario get' está disponible por ahora.");
            }
                ...
                if (param != null) {
                    try {
                        ...
                        sendTableResponse(senderEmail, "Producto encontrado", 
                            DProducto.HEADERS, (ArrayList<String[]>) productoData, comando);
                    } catch (NumberFormatException e) {
                        sendSimpleResponse(senderEmail, "Error", "ID inválido: " + param);
                    }
                } else {
                    ...
                    sendTableResponse(senderEmail, "Lista de Productos", 
                        DProducto.HEADERS, (ArrayList<String[]>) productos, comando);
                }
            } else {
                sendSimpleResponse(senderEmail, "Comando no disponible", 
                    "Solo 'producto get' está disponible por ahora.");
            }
        } catch (SQLException ex) {
                ...
                if (param != null) {
                    try {
                        ...
                        sendTableResponse(senderEmail, "Categoría encontrada", 
                            DCategoria.HEADERS, (ArrayList<String[]>) categoriaData, comando);
                    } catch (NumberFormatException e) {
                        sendSimpleResponse(senderEmail, "Error", "ID inválido: " + param);
                    }
                } else {
                    ...
                    sendTableResponse(senderEmail, "Lista de Categorías", 
                        DCategoria.HEADERS, (ArrayList<String[]>) categorias, comando);
                }
            } else {
                sendSimpleResponse(senderEmail, "Comando no disponible", 
                    "Solo 'categoria get' está disponible por ahora.");
            }
        } catch (SQLException ex) {
    ...
    private void processHelpCommand(String senderEmail, String comando) {
        try {
            ...
            data.add(new String[]{"categoria get <id> / categorias get <id>", "✅ SÍ", "Categoría específica por ID"});
            ...
            data.add(new String[]{"tipo_pago get <id> / tipos_pago get <id>", "✅ SÍ", "Tipo de pago específico por ID"});
            
            // Comandos de administración (próximamente)
            data.add(new String[]{"producto add nombre precio_compra precio_venta descripcion categoria_id", "⏳ DESARROLLO", "Agregar nuevo producto"});
            ...
            data.add(new String[]{"✅ Comandos en SINGULAR y PLURAL", "NUEVO", "usuario = usuarios, producto = productos"});
            ...
            data.add(new String[]{"✅ Comandos en CONTENIDO de email", "NUEVO", "Puedes escribir comandos en el texto"});
            ...
            sendErrorEmail(senderEmail, "Error: " + ex.getMessage());
        }
    }
                ...
                if (param != null) {
                    try {
                        ...
                        sendTableResponse(senderEmail, "Cliente encontrado", 
                            DCliente.HEADERS, (ArrayList<String[]>) clienteData, comando);
                    } catch (NumberFormatException e) {
                        sendSimpleResponse(senderEmail, "Error", "ID inválido: " + param);
                    }
                } else {
                    ...
                    sendTableResponse(senderEmail, "Lista de Clientes", 
                        DCliente.HEADERS, (ArrayList<String[]>) clientes, comando);
                }
            } else {
                sendSimpleResponse(senderEmail, "Comando no disponible", 
                    "Solo 'cliente get' está disponible por ahora.");
            }
        } catch (SQLException ex) {
                ...
                if (param != null) {
                    try {
                        ...
                        sendTableResponse(senderEmail, "Tipo de pago encontrado", 
                            DTipoPago.HEADERS, (ArrayList<String[]>) tipoPagoData, comando);
                    } catch (NumberFormatException e) {
                        sendSimpleResponse(senderEmail, "Error", "ID inválido: " + param);
                    }
                } else {
                    ...
                    sendTableResponse(senderEmail, "Lista de Tipos de Pago", 
                        DTipoPago.HEADERS, (ArrayList<String[]>) tiposPago, comando);
                }
            } else {
                sendSimpleResponse(senderEmail, "Comando no disponible", 
                    "Solo 'tipo_pago get' está disponible por ahora.");
            }
        } catch (SQLException ex) {
    ...
    public void usuario(ParamsAction event) {
        // Redirigir al procesamiento directo
        processUsuarioCommand(event.getSender(), "get", null, event.getCommand());
    }
    ...
    public void evento(ParamsAction event) {
        sendSimpleResponse(event.getSender(), "Comando no disponible", 
            "El comando 'evento' aún no está implementado en el servidor independiente.");
    }

    @Override
    public void reserva(ParamsAction event) {
        sendSimpleResponse(event.getSender(), "Comando no disponible", 
            "El comando 'reserva' aún no está implementado en el servidor independiente.");
    }

    @Override
    public void pago(ParamsAction event) {
        sendSimpleResponse(event.getSender(), "Comando no disponible", 
            "El comando 'pago' aún no está implementado en el servidor independiente.");
    }

    @Override
    public void proveedor(ParamsAction event) {
        sendSimpleResponse(event.getSender(), "Comando no disponible", 
            "El comando 'proveedor' aún no está implementado en el servidor independiente.");
    }

    @Override
    public void promocion(ParamsAction event) {
        sendSimpleResponse(event.getSender(), "Comando no disponible", 
            "El comando 'promocion' aún no está implementado en el servidor independiente.");
    }

    @Override
    public void patrocinador(ParamsAction event) {
        sendSimpleResponse(event.getSender(), "Comando no disponible", 
            "El comando 'patrocinador' aún no está implementado en el servidor independiente.");
    }

    @Override
    public void patrocinio(ParamsAction event) {
        sendSimpleResponse(event.getSender(), "Comando no disponible", 
            "El comando 'patrocinio' aún no está implementado en el servidor independiente.");
    }

    @Override
    public void rol(ParamsAction event) {
        sendSimpleResponse(event.getSender(), "Comando no disponible", 
            "El comando 'rol' aún no está implementado en el servidor independiente.");
    }
    ...
    public void servicio(ParamsAction event) {
        ...
        try {
                    ...
                    if (event.getParams() != null && !event.getParams().isEmpty()) {
                        ...
                        try {
                            ...
                            sendTableResponse(event.getSender(), "Categoría encontrada", 
                                DCategoria.HEADERS, (ArrayList<String[]>) categoriaData, event.getCommand());
                        } catch (NumberFormatException e) {
                            sendSimpleResponse(event.getSender(), "Error", "ID de categoría inválido.");
                        }
                    } else {
                        List<String[]> categorias = dCategoria.list();
                        sendTableResponse(event.getSender(), "Lista de Categorías", 
                            DCategoria.HEADERS, (ArrayList<String[]>) categorias, event.getCommand());
                    }
                    break;
                default:
                    sendSimpleResponse(event.getSender(), "Comando no disponible", 
            ...
            handleError(DATABASE_ERROR, event.getSender(), Collections.singletonList("Error de base de datos: " + ex.getMessage()));
        }
    }
    ...
    public void detalleEvento(ParamsAction event) {
        sendSimpleResponse(event.getSender(), "Comando no disponible", 
            "El comando 'detalleEvento' aún no está implementado en el servidor independiente.");
    }
     ...
     */
    private void sendSimpleResponse(String email, String title, String message) {
        String content = HtmlRes.generateText(new String[]{title, message});
        emailRelay.sendEmail("servidor-independiente@localhost", email, "[Servidor Independiente] " + title, content);
    }
    ...
    /**
     * Envía respuesta con tabla y comando personalizado
     */
    private void sendTableResponse(String email, String title, String[] headers, ArrayList<String[]> data, String command) {
        String content = HtmlRes.generateTable(title, headers, data);
    ...
    /**
     * 🆕 Envía respuesta como reply a un email específico
     */
    private void sendTableResponseAsReply(String email, String title, String[] headers, ArrayList<String[]> data, String command, String originalSubject, String messageId) {
        String content = HtmlRes.generateTable(title, headers, data);
        String subject = command != null ? "[Comando: " + command + "] " + title : title;
        ...
        if (messageId != null && !messageId.trim().isEmpty()) {
            ...
            emailRelay.sendEmail("servidor-independiente@localhost", email, subject, content);
            System.out.println("   📧 Respuesta enviada como email nuevo");
        }
    }

    /**
     * 🆕 Envía respuesta simple como reply a un email específico
     */
    private void sendSimpleResponseAsReply(String email, String title, String message, String originalSubject, String messageId) {
        String content = HtmlRes.generateText(new String[]{title, message});
        
        if (messageId != null && !messageId.trim().isEmpty()) {
            emailRelay.replyToEmail(email, originalSubject, content, messageId);
            System.out.println("   📧 Respuesta simple enviada como REPLY al email original");
        } else {
            // Fallback al método normal
            emailRelay.sendEmail("servidor-independiente@localhost", email, "[Servidor Independiente] " + title, content);
            System.out.println("   📧 Respuesta simple enviada como email nuevo");
        }
    }
    ...
    private void sendErrorEmail(String email, String error) {
        sendSimpleResponse(email, "Error de Procesamiento", error);
    }
